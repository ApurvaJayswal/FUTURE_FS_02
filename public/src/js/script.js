...
function updateCurrentWeather(data) {
  ...
  // Change wind speed display to km/h for metric, mph for imperial
  const speedUnit = currentUnit === "metric" ? "km/h" : "mph"
  const speedValue = currentUnit === "metric" ? (data.wind.speed * 3.6).toFixed(1) : (data.wind.speed * 2.237).toFixed(1)
  ...
  // // Uncomment the following lines to set a background image based on the location 
  // const query = country ? `${locationName} ${country}` : locationName;
  // fetch(`https://api.unsplash.com/photos/random?query=${encodeURIComponent(query)}&orientation=landscape&client_id=1JKh2_A9YNmFrMVr4WLd-hzpqY9hFSyGxqkgB0zQv8U`)
  //   .then(response => response.json())
  //   .then(photoData => {
  //     if (photoData && photoData.urls && photoData.urls.full) {
